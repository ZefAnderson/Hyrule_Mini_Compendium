...
class compendium {
     ...
     */
    get_entry(entry, callback, timeout = this.default_timeout, error_callback = (err) => { throw (err); }) {
        let req = https.get(this.url + "/entry/" + String(entry), (resp) => {
            let data = "";
            resp.on("data", (chunk) => {
                data += chunk;
            });
            resp.on("end", () => {
                ...
                if (Object.keys(entry_data).length === 0) {
                    throw new exceptions_1.NoEntryError(entry);
                }
                if (entry_data.category === "monsters") {
                    ...
                    throw new exceptions_1.NoCategoryError(entry_data.category);
                }
            });
     ...
     * @param {Function} [error_callback=(err)=>{throw(err)] Function to be executed on error
     */
    get_category(category, callback, timeout = this.default_timeout, error_callback = (err) => { throw err; }) {
        let req = https.get(this.url + "/category/" + String(category), (resp) => {
            let data = "";
            resp.on("data", (chunk) => {
                data += chunk;
            });
            resp.on("end", () => {
                ...
                if (Object.keys(categoryData).length === 0) {
                    throw new exceptions_1.NoCategoryError(category);
                }
                callback(types_1.type_category(categoryData));
            });
        }).on("error", error_callback);
        req.setTimeout(timeout, req.destroy);
    }
     ...
     * @param {Function} [error_callback=(err)=>{throw(err)] Function to be executed on error
     */
    get_all(callback, timeout = this.default_timeout, error_callback = (err) => { throw err; }) {
        let req = https.get(this.url, (resp) => {
            let data = "";
            resp.on("data", (chunk) => {
                data += chunk;
            });
            resp.on("end", () => {
                ...
                for (const categoryName in allData) {
                    allData[categoryName] = types_1.type_category(allData[categoryName]);
                }
                callback(allData);
            });
        }).on("error", error_callback);
     ...
     */
    get_entry_image(entry) {
        return new types_1.EntryImage(this, entry);
    }
    /**
     ...
     * @param {Function} [error_callback=(err)=>{throw(err)] Function to be executed on error
     */
    download_entry_image(entry, output_file, callback = () => { }, timeout = this.default_timeout, error_callback = (err) => { throw err; }) {
        this.get_entry(entry, (data) => {
